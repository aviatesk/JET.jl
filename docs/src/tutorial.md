...
analysis.
Making your code type stable lets JET's type error analysis cover more of it.
...
As mentioned above, eliminating or reducing dynamic dispatch with
`@report_opt` keeps the Julia compiler from being blinded by dynamic dispatch
and thereby widens the reach of JET's analysis.
