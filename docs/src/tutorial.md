...
instability and type errors.
This tutorial will demonstrate how to use JET effectively.
It presupposes the reader has working knowledge of Julia, and understands Julian
concepts such as "dynamic dispatch" and "type stability" - see the
[Julia documentation](https://docs.julialang.org/en/v1/) for
explanation of these concepts.

Because JET relies on the compiler's type inference, it is not able to
effectively analyze type unstable code.
Making your code type stable is a prerequisite for effectively using JET's type
error analysis.
...
So you can just install it via Julia's built-in package manager and use it just
...
The dynamic dispatch we see here come from the fact that `func_var` is an
...
Sometimes, type instability only shows up much deeper into a call chain,
several functions deep.
...
As mentioned above, effective use of JET begins with liberal use of
`@report_opt` to eliminate or reduce any dynamic dispatch, such that the Julia
compiler is not blinded by dynamic dispatch.
...
just like `(@)report_opt` - but where the latter reports dynamic dispatch, the
former finds type errors.
...
Note also that the two possible errors shown are mutally exclusive - no input
...
This illustrates that JET does not analyze _methods_, but rather _function
calls with specific types_. More precisely, JET analyses so-called
_methodinstances_, since methodinstances are compiled whereas methods are not.
...
In the example above, if your code calls `f(nothing)`, the error may originate
from Base, but it's clearly an error in your own code to call `f(nothing)`.
