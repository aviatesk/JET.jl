...
So far, we have used the default error analysis pass, which collects problems
according to one specific, somewhat opinionated definition of "errors". JET
offers other error-reporting passes, including "sound" error detection and the
simpler "typo" detection pass. They can be switched using the `mode`
configuration:
...
This error occurs when a variable `x` is inferred to be a union type, and one
or more of the union's members would lead to a `MethodError`. For example, if
the compiler infers `x` to be of type `Union{A, B}` and then calls `f(x)`, this
error occurs if the call would lead to a `MethodError` when `x` is an `A`.
...
In addition to the [general configurations](@ref), error analysis can take the
