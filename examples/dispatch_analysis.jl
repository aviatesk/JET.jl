...
function CC.finish!(analyzer::DispatchAnalyzer, frame::CC.InferenceState, validation_world::UInt, time_before::UInt64)
    ...
    if analyzer.frame_filter(frame.linfo)
        ...
        elseif isnothing(src) # means, compiler decides not to do optimization
            report_optimization_failure_dispatch(analyzer, caller, src)
        elseif isa(src, CC.OptimizationState) # the compiler optimized it, analyze it
            report_runtime_dispatch_dispatch(analyzer, caller, src)
...
function report_optimization_failure_dispatch(analyzer::DispatchAnalyzer, result::CC.InferenceResult, src)
...
function report_runtime_dispatch_dispatch(analyzer::DispatchAnalyzer, caller::CC.InferenceResult, opt::CC.OptimizationState)
