...
function maybe_profile_builtin_call(frame, call_expr, expand::Bool)
  ...
  # HACK:
  # - builtins and intrinsics have empty method tables -- we can circumvent the
  #   long "switch" check by looking for this.
  # - we should special case our wrapper `IntrinsicFunctionType` type here
  if !isa(f_type, IntrinsicFunctionType)
    mt = f_type.name.mt
    isa(mt, Core.MethodTable) && !isempty(mt) && return call_arg_types
  end

  f = to_function(f_type)
  isa(f, Function) || error("can't lookup f correctly: $(f)")
