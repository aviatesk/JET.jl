...
function print_report(io::IO, frame::Frame; view = :inline)
    ...
    if view === :inline
        ...
        for er in reports
            depth =
                print_calltrace(io, er.lin, er.frame, wrote_lins; duplicate_lines = false)
...
report_string(er::ErrorReport) =
    return error("report_string(::$(typeof(er))) should be implemented")

function report_string(er::UndefVarErrorReport)
    be = er.maybe ? "may not be" : "is not"
    return if isa(er.mod, Module)
...
report_string(er::InvalidBuiltinCallErrorReport) = "invalid builtin function call: $(tt_to_signature_str(er.tt))"
report_string(er::NoMethodErrorReport) = "no method matching signature: $(tt_to_signature_str(er.tt))"
report_string(er::ConditionErrorReport) = "non-boolean ($(er.t)) found in boolean context"
...
print_calltrace(io, lin, frame, wrote_lins::Set{UInt64} = Set{UInt64}(); kwargs...) =
    return _print_calltrace(io, lin, lin, frame, wrote_lins; kwargs...)

function _print_calltrace(
    io, lin, err_lin, frame, wrote_lins::Set{UInt64} = Set{UInt64}();
    ...
    # prewalk
    depth = _print_calltrace(
        io, prev_lin, err_lin, prev_frame, wrote_lins;
...
function print_location(io, lin, err_lin, depth)
    ...
    color = lin == err_lin ? ERROR_COLOR : RAIL_COLORS[(depth+1)%length(RAIL_COLORS)+1]
    printstyled(io, "┌ @ ", lin.file, ":", lin.line; color)
...
basepath(filename) =
    return normpath(joinpath((@static isdir(SRC_DIR) ? SRC_DIR : RELEASE_DIR), filename))
